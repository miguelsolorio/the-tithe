...
- `L.prop(name, x, z, { y, face: 'n'|'s'|'e'|'w' | rotY, scale, fallen: 'side'|'back'|'front', dynamic, flicker, buzz, collider, args })`: props from `src/world/props` (`PROP_NAMES` lists them; `args` go to the prop builder). Props face +Z by default (`face: 's'`). Wall props sit on the wall surface (walls are 0.2 m thick, centred on the cell edge, so a wall on the line x = 5 has its surface at x = 4.9 / 5.1). Returns the object (`userData.colliders`, `userData.lever`, `userData.slot`, ...). Props listed in `src/world/props/interactive.js` (chairs, crates, barrels, bookshelves, shelves, ...) react to the player: walking into them shoves them, sprinting or hitting them can tip them over, and stabbing or shooting a shelf knocks books and jars off it (`src/systems/props.js`). They stay batched until touched. Pass `interactive: false` to keep one fixed. A prop with another prop or a pickup standing on it is kept fixed automatically.
